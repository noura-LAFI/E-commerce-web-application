...
$x=count($_SESSION["panier"]);
$x--;
$con = mysqli_connect("localhost","root","","shopping") or die("erreur base");
for($i=1;$i <= $x;$i++){
$id = $_SESSION["panier"][$i];
$req = "select * from articles where idArt=$id";
$resultat = mysqli_query($con , $req) or die ("erruer requete ");
while($ligne = mysqli_fetch_array($resultat,MYSQLI_NUM)){
        echo("<tr>
        <td class='align-middle'><img src='img/$ligne[7]' alt='' style='width: 50px;'> $ligne[1]</td>
                            <td class='align-middle'> $ligne[2]</td>
                            ...
                            <td class='align-middle'>   <button name=btn1 
                             class='btn btn-sm btn-primary'> <i class='fa fa-times'></i></button> <form></td>
